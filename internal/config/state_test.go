...
func TestStateConfigurationMigration(t *testing.T) {
	cfg, warnings, err := Migrate([]byte(`{"server":{"state":{"instanceId":"child-one","parentUrl":"https://parent/v1/op/state","parentEventsUrl":"https://parent/v1/op/events","parentToken":"secret","refreshInterval":"2s","heartbeatInterval":"10s","staleAfter":"1m"}}}`))
	if err != nil {
		t.Fatal(err)
	}
	if len(warnings) != 0 {
	...
	s := cfg.Server.State
	if s.InstanceID != "child-one" || s.ParentURL != "https://parent/v1/op/state" || s.ParentEventsURL != "https://parent/v1/op/events" || s.ParentToken != "secret" || s.RefreshInterval.Duration != 2*time.Second || s.HeartbeatInterval.Duration != 10*time.Second || s.StaleAfter.Duration != time.Minute {
