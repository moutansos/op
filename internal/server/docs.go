...
var openAPIDocument = []byte(`{
  ...
  "components": {
    ...
    "schemas": {
      ...
      "ParentCommand":{"type":"object","required":["version","type"],"description":"JSON in SSE data lines on parentEventsUrl. Child GET with parent bearer token and X-Op-Instance-Id. Ignore other instanceIds and duplicate eventIds.","properties":{"version":{"type":"integer","enum":[1]},"instanceId":{"type":"string"},"eventId":{"type":"string"},"type":{"type":"string","enum":["command.open_project","command.select_pane","command.ping"]},"payload":{"type":"object","properties":{"projectId":{"type":"string"},"nativeId":{"type":"string"},"profile":{"type":"string"},"newInstance":{"type":"boolean"},"paneId":{"type":"string"}}}}},
  ...
  "paths": {
    ...
    "/v1/tmux/panes/{id}/select": {"post": {"security": [{"bearerAuth": []}], "summary": "Select a managed tmux pane", "parameters": [{"name": "id", "in": "path", "required": true, "schema": {"type": "string"}, "description": "Canonical tmux pane ID such as %12"}], "responses": {"200": {"description": "Selected"}, "404": {"description": "Pane not found"}}}},
    "/v1/stats": {"get": {"security": [{"bearerAuth": []}], "summary": "Get host, process, and agent classification snapshot", "responses": {"200": {"description": "Snapshot"}}}},
